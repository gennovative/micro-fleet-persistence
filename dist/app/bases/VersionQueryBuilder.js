...
class VersionQueryBuilder {
    ...
    buildPatch(entity, prevQuery, rawQuery, opts) {
        return rawQuery.patch(entity).whereComposite(this._EntityClass['idColumn'], '=', this._toArr(entity, this._EntityClass['idProp'])).where('is_main', true);
    }
    buildRecoverOpts(pk, prevOpts, rawOpts) {
        return prevOpts;
    }
    buildUpdate(entity, prevQuery, rawQuery, opts) {
        return rawQuery.update(entity).whereComposite(this._EntityClass['idColumn'], '=', this._toArr(entity, this._EntityClass['idProp'])).where('is_main', true);
    }
    _toArr(pk, arr) {
